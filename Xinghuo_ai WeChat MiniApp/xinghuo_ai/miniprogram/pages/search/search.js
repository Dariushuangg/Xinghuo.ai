...
Page({
  ...
  test:function(){
    ...
    app.globalData.has_BLEConnected = true
  },
  ...
  Connect: function (e) {
    ...
    for (var i = 0; i < that.data.devicesList.length; i++) {
      if (e.currentTarget.id == that.data.devicesList[i].deviceId) {
        ...
        app.globalData.name = that.data.devicesList[i].name
        console.log(name)
    ...
    wx.createBLEConnection({
      ...
      success: function (res) {
        ...
        wx.redirectTo({
          url: '../index/index'
          //'../device/device?connectedDeviceId=' + e.currentTarget.id + '&name=' + name
        })
        app.globalData.has_BLEConnected = true
      },
      fail: function (res) {
